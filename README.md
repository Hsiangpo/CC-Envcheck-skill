...
CC-Check 是一个面向 **Claude Code / API 开发者** 的终端环境审计工具。它通过 40+ 项自动化检测，识别你的终端环境中可能导致 API 风控（被 ban、限速、封号）的风险信号，并提供一键修复能力。
...
| **DNS** | 15/100 | Google DNS whoami、Cloudflare DNS、系统 DNS 展示值 |
| **系统** | 21/100 | 时区、语言、代理环境变量、输入法、主机名、度量单位 |
| **网络** | 10/100 | 公网 IP 可达性、多源 IP 一致性、IPv6 泄露 |
| **Clash** | 8/100 | 进程检测、模式、TUN 开启、配置标记、DNS 守护 |
| **包管理器** | 6/100 | npm / pip / brew 中国镜像残留 |
| **隐私** | 6/100 | Claude 遥测目录、隐私环境变量、Shell 历史 |
...
| 系统 DNS 展示值 | `networksetup Empty` / `resolvectl revert` / `Set-DnsClientServerAddress -Reset` |
...
| DNS 守护 | macOS LaunchAgent / Linux systemd user timer |
...
|------|-------|-------|---------|
| 全维度检测 | ✅ 完整 | ✅ 完整 | ✅ 完整 |
| DNS 修复 | ✅ `networksetup` | ✅ `resolvectl` | ✅ `PowerShell` |
| DNS 守护 | ✅ LaunchAgent | ✅ systemd timer | ❌ |
...
```bash
pip3 install paramiko  # 仅 VPN 远端检查需要，可选
```
...
```bash
# 1. 全面审计（推荐）
python3 scripts/cc_check.py inspect

# 2. JSON 输出（便于自动化）
python3 scripts/cc_check.py inspect --json

# 3. 预览修复（不实际执行）
python3 scripts/cc_check.py fix-local --dry-run

# 4. 执行修复
python3 scripts/cc_check.py fix-local

# 5. 完整闭环：检测 → 修复 → 验证
python3 scripts/cc_check.py full
...
| 模块 | 行数 | 职责 |
|------|------|------|
| `cc_check.py` | ~880 | CLI 解析、审计流程编排、修复逻辑、结果输出 |
| `platform_ops.py` | ~720 | 跨平台抽象：DNS / TUN / 进程 / 输入法 / Shell 等 |
...
```bash
# 运行全部 36 个单元测试
python3 -m unittest tests/test_cc_check.py -v
